jQuery(document).ready(function ($) {
    ...
    window.wc_state = $('select#billing_state');
    window.current_wc_state = window.wc_state.val();
    window.wc_city = $('input#billing_city');
    window.current_wc_city = window.wc_city.val();

    function Customer_Address_iran_Cities(province) {
    ...
    function customer_address_setup_city_box(state, city = '', replace = false) {


        let list = Customer_Address_iran_Cities(state);

        if (replace === true) {

            let citySelect = '<select name="billing_city" id="billing_city" class="city_select" autocomplete="address-level1" data-placeholder="یک گزینه انتخاب نمائید…" data-input-classes="" data-label="شهر" tabindex="-1" aria-hidden="true">';
            list.forEach(function (item) {
                citySelect += '<option value="' + item[0] + '">' + item[0] + '</option>';
            });
            citySelect += '</select>';
            window.wc_city.replaceWith(citySelect);
            $('select#billing_city').select2().val(city).trigger("change");
        } else {

            let options = [];
            list.forEach(function (item) {
                options.push({id: item[0], text: item[0]});
            });

            if (city == null) {
                city = options[0]['id'];
            }
            $('select#billing_city').empty().select2({
                data: options
            }).val(city).trigger('change');
        }
    }

    customer_address_setup_city_box(window.current_wc_state, window.current_wc_city, true);

    jQuery(document).on("change.select2", 'select#billing_state', function (e) {
        e.preventDefault();
        let value = $("#billing_city").val();
        if (typeof window.wc_change_city !== 'undefined' && window.wc_change_city.length > 0) {
            value = window.wc_change_city;
        }
        window.wc_change_city = '';
        customer_address_setup_city_box(jQuery(this).val(), value, false);
    });
});